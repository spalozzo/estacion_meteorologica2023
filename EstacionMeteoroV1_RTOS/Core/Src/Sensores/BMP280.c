...
extern float TemperatureBMP, PressureBMP, HumidityBMP;
extern uint8_t MemRxCallback_Function;
...
int BMEReadRaw(void)
{
	...
	//MemRxCallback_Function= READ_CHIP_ID;
	//HAL_I2C_Mem_Read_IT(&hi2c1, BME280_ADDRESS, ID_REG, 1, &chipID, 1);
	...
	if (chipID == 0x58) // original era 0x60 pero en BMP280 es 0x58
	{
		...
		// Ahora esto de aca abajo se esta calculando en la callback de I2C
...
#elif SUPPORT_32BIT
			  PressureBMP = (BME280_compensate_P_int32 (pRaw));  // as per datasheet, the pressure is Pa
